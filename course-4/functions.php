...
function getRating(){
    ...
    if( $totalGuesses > 0 ){
        for ( $i=0; $i < 5 ; $i++ ){ // compare rating to the grades, 
            if( $rating >= $grade[$i][0] )  {
                return $grade = [$grade[$i][1], $grade[$i][2]]; // return a grade with the correct value and colour
            }
...
function guessesPost() {
    if(isset($_POST['guess'])){
        if(!in_array($_POST['guess'], $_SESSION['guesses']) ){   // if the letter is not in the word check it
            if(strpos( $_SESSION['word'], $_POST['guess']) === FALSE){ // if they do not match is an incorrect guess
                $_SESSION['lives']--;
